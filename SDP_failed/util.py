	...
	with open(path, 'r', encoding='utf8') as f:
		...
		for item in items:
			if item == '':
				continue
			form, lemma, pos = [], [], []
			lines = item.strip().lower().split('\n')[1:]
			for line in lines:
				line = line.split('\t')
				form.append(line[1])
				lemma.append(line[2])
				pos.append(line[3])
			datas.append([form, lemma, pos])
	return datas
	...
	with open(path, 'r', encoding='utf8') as f:
		...
		for item in items:
			...
			for line in lines:
				...
				pred.append(line[5])
				arg.append(line[6:])
			datas.append([form, lemma, pos, top, pred, arg])
	return datas
...
def data_transform(data):
	...
	for i in range(len(forms)):
		word_based_data.append([forms[i], lemmas[i], poss[i], tops[i], preds[i], args[i]])
	'''
...
def gen_data():
	...
	with open('lemmas', 'w', encoding='utf8') as f:
		for lemma in lemmas:
			f.write(lemma+'\n')
	with open('poss', 'w', encoding='utf8') as f:
		for pos in poss:
			f.write(pos+'\n')
